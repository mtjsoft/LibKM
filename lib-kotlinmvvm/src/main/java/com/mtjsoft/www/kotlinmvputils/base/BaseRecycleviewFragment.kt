...
abstract class BaseRecycleviewFragment<T, VM : BaseRecycleViewModel<T>> : BaseTopViewFragment(),
    AdapterItemClickListener, LoadViewImp {

    lateinit var viewModel: VM

    override fun layoutResId(): Int {
        return R.layout.hh_activity_recycleview
    }
    ...
    private fun initValues() {
        ...
        viewModel.linearLayoutManager.value = LinearLayoutManager(context)
        viewModel.gridLayoutManager.value = GridLayoutManager(context, setCount())
        viewModel.staggeredGridLayoutManager.value = StaggeredGridLayoutManager(
            setCount(),
            StaggeredGridLayoutManager.VERTICAL
        )
        if (setItemDecoration() > 0) {
            val itemPad = AndDensityUtils.dip2px(context, setItemDecoration())
            // 设置间隔
